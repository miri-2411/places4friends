import ResetPasswordForm from "@/components/ResetPasswordForm";
import LegalFooter from "@/components/LegalFooter";
import { Lock } from "lucide-react";
import type { Metadata } from "next";
import { createClient } from "@/lib/supabase/server";
import { redirect } from "next/navigation";

export const metadata: Metadata = {
  title: "Passwort zurücksetzen",
  description: "Setze dein Passwort auf places4friends zurück.",
};

export default async function ResetPasswordPage() {
  const supabase = await createClient();
  const { data: { user } } = await supabase.auth.getUser();

  if (!user) {
    redirect(
      "/login?error=access_denied&error_description=Bitte+fordere+einen+neuen+Passwort-Zur%C3%BCcksetzen-Link+an."
    );
  }

  return (
    <div className="flex flex-col min-h-screen bg-slate-50/50 pb-20 font-sans">
      {/* Header */}
      <header className="sticky top-0 z-10 flex h-14 items-center justify-center border-b border-slate-100 bg-white px-4">
        <h1 className="text-lg font-bold text-slate-900">Passwort zurücksetzen</h1>
      </header>

      <div className="flex-grow overflow-y-auto px-5 pt-8 page-transition">
        {/* App Brand */}
        <div className="flex flex-col items-center text-center mb-8">
          <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-gradient-to-br from-brand-green-500 to-brand-green-700 text-white shadow-lg shadow-brand-green-500/20 mb-4">
            <Lock className="h-7 w-7" />
          </div>
          <h2 className="text-xl font-bold text-slate-900">
            Neues Passwort festlegen
          </h2>
          <p className="mt-1.5 text-xs text-slate-500 max-w-[260px]">
            Wähle ein neues Passwort für dein Konto, um dich wieder anzumelden.
          </p>
        </div>

        <div className="mx-auto max-w-sm rounded-2xl border border-slate-100 bg-white p-5 shadow-[0_8px_30px_rgb(0,0,0,0.03)]">
          <ResetPasswordForm />
        </div>

        <LegalFooter />
      </div>
    </div>
  );
}
